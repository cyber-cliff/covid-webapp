...
def job_function():
    ...
    if os.path.exists(path):
        ...
        for key,value in df.iterrows():
            ...
            try:
                fields["service_level"] = int(value.service_level[-1])
            except:
                fields["service_level"] = None
            
            try:
                fields["capacity"] = int(value.capacity)
            except:
                fields["capacity"] = None

            if value.covid_capacity.lower() == 'yes':
                fields["covid_capacity"] = True
            else:
                fields["covid_capacity"] = False
